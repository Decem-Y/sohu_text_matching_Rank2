...
class NezhaConfig(object):
    ...
    def from_dict(cls, json_object):
        """Constructs a `NezhaConfig` from a Python dictionary of parameters."""
        config = NezhaConfig(vocab_size_or_config_json_file=-1)
    ...
    def from_json_file(cls, json_file):
        """Constructs a `NezhaConfig` from a json file of parameters."""
...
class NeZhaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        #     length=512, depth=self.attention_head_size, max_relative_position=config.max_relative_position).to(
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, NezhaConfig):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class `NezhaConfig`. "
