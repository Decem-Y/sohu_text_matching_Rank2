...
from data import SentencePairDatasetWithType, SentencePairDatasetForSBERT, SentencePairDatasetWithMultiType
...
def eval(model, device, test_dataloader, criterion_type='CE'):
    ...
    for idx, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            all_loss = None
            for task_id in range(num_tasks):
                # print(task_id, all_masks[task_id])
                if all_masks[task_id].sum() != 0:
                    if all_loss is None:
                        all_loss = criterion(all_output[task_id], all_labels[task_id])
                    else:
...
if __name__ == '__main__':
    ...
    # distinguish model architectures or pretrained models according to model_type
        ...
        if 'nezha' in model_type.lower():
            ...
            model = BertClassifierSingleModel(bert_dir=pretrained, task_num=6, hidden_size=hidden_size)
        model.to(device)
        ...
        train_dataset = SentencePairDatasetWithMultiType(train_data_dir, True, pretrained, shuffle_order, aug_data=aug_data, clip=clip_method)
        train_dataloader = DataLoader(train_dataset, batch_size=train_bs, shuffle=True)

        print("Loading Dev Data...")
        # test_dataset = SentencePairDatasetWithType(dev_data_dir, True, pretrained, clip=clip_method)
        test_dataset = SentencePairDatasetWithMultiType(dev_data_dir, True, pretrained, clip=clip_method)
        test_dataloader = DataLoader(test_dataset, batch_size=eval_bs, shuffle=False)
